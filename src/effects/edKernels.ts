...
export const defaults = {
  kernel: 'sierraLite' as 'sierraLite' | 'twoRowSierra' | 'stevensonArce' | 'shiauFan',
  serpentine: true,
  clip: true,
  gain: 1
};
...
const KERNELS: Record<string, { taps: KernelTap[]; div: number }> = {
  sierraLite: {
    div: 4,
    taps: [
      { dx: 1, dy: 0, w: 2 },
...
export function apply(
  ...
  const kernel = getKernel(params?.kernel || defaults.kernel);
  const serpentine = params?.serpentine !== undefined ? Boolean(params.serpentine) : defaults.serpentine;
  const clip = params?.clip !== undefined ? Boolean(params.clip) : defaults.clip;
  const gain = Math.max(0, Number(params?.gain ?? defaults.gain));
