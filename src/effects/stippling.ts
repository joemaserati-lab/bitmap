...
function drawDisc(buffer: Uint8ClampedArray, width: number, height: number, cx: number, cy: number, radius: number) {
...
export function apply(
  ...
  const minR = clamp(Number(params?.minR ?? defaults.minR), 0.5, 10);
  const maxR = clamp(Number(params?.maxR ?? defaults.maxR), minR, 12);
  const density = clamp(Number(params?.density ?? defaults.density), 0.1, 4);
