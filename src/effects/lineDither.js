...
export function apply(img, params, _ctx, cache) {
  const cell = Math.max(2, Number((params && params.cell) ?? defaults.cell) || 8);
  const angle = Number((params && params.angle) ?? defaults.angle) || 0;
  const thickness = clamp(Number((params && params.thickness) ?? defaults.thickness), 0.05, 1);
  const aa = params && params.aa !== undefined ? Boolean(params.aa) : defaults.aa;
